...
from views._common import handle_garmin_errors, render_session_recommendation
...
def render() -> None:
    ...
    with col1:
        ...
        st.bar_chart(pd.Series(weekly_volume(sessions, "distanse_km"), name="km"))
    with col2:
        st.caption("Lengste langtur per uke (km) — progresjon mot 70.3-distansen")
        st.bar_chart(pd.Series(weekly_longest(sessions, "distanse_km"), name="km"))

    st.divider()

    col3, col4 = st.columns(2)
    with col3:
        st.caption("Tempo per økt (min/km) — lavere er raskere")
        pace_series = pd.Series(
            {s["dato"][:10]: s["tempo_min_per_km"] for s in sessions if s.get("tempo_min_per_km")}
        ).sort_index()
        pace_series.index = pd.to_datetime(pace_series.index)
        st.line_chart(pace_series)
    with col4:
